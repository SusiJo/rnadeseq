...
if (isProvided(params$path_contrast_pairs)){
    for (i in 1:length(contrast_names)){
        conditionA <- sapply(strsplit(contrast_names[i], '_'), function(x) paste(x[2], collapse=' '))
        conditionB <- sapply(strsplit(contrast_names[i], '_'), function(x) paste(x[3], collapse=' '))
        cat(paste("* Interaction effect between", conditionA, "and", conditionB, "\n"))
    ...
    cat(paste("The expression profile of the first group (treatment) was compared to the second group (control)."))
}
...
for (file in allgenes_files){
    ...
    if (isProvided(params$path_contrast_pairs)){
        # DE_contrast_interaction_conditionA_conditionB --> conditionA vs conditionB
        current_contrast_name <- sapply(strsplit(fname, '_'), function(x) paste(x[4:contrast_len], collapse=' '))
    } else {
...
for (i in 1:length(volcano_list)){
    if (isProvided(params$path_contrast_pairs)){
         cat("#### Interaction: ", contrast_list[[i]], "\n")
         cat("\n")
        baseline_contrast <- sapply(strsplit(contrast_list[i], ' '), function(x) paste(x[2], collapse=' '))
...
for (file in contrast_files){
    ...
    if (isProvided(params$path_contrast_pairs)){
        # DE_contrast_interaction_conditionA_conditionB --> conditionA vs conditionB
        contrast_list <- sapply(strsplit(fname, '_'), function(x) paste(x[4:contrast_len], collapse=' '))
    } else {
        # DE_contrast_condition_conditionName_conditionA_vs_conditionB --> conditionA vs conditionB
        contrast_list <- sapply(strsplit(fname, '_'), function(x) paste(x[5:contrast_len], collapse=' '))
    }
}
...
for (file in allgenes_files) {
    ...
    if (isProvided(params$path_contrast_pairs)){
        conditionA <- sapply(strsplit(fname, '_'), function(x) paste(x[2], collapse=' '))
        conditionB <- sapply(strsplit(fname, '_'), function(x) paste(x[3], collapse=' '))
        cat(paste("* Interaction effects between", conditionA, "and", conditionB, "have", numDE, "DE genes out of", num_all, "tested genes.\n"))
    } else{
